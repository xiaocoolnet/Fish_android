...
public class RegisterActivity extends Activity implements View.OnClickListener {
    ...
    private void VerifyCode() {
        ...
        if(phone.length() == 11) {
            ...
            Toast.makeText(RegisterActivity.this,"手机号输入不合法",Toast.LENGTH_SHORT).show();
        }
    }
    ...
    private void Next() {
        ...
        if(phone.equals("18653503680") && yzCode.equals("123456") && phone.length()==11 && yzCode.length()==6){ // 成功验证
            Toast.makeText(RegisterActivity.this,"验证成功，开始设置密码",Toast.LENGTH_SHORT).show();

            // 将手机号传送到下一个Activity显示
            Intent getPhoneIntent = new Intent();
            getPhoneIntent.setClass(RegisterActivity.this, SetPasswordActivity.class);
            getPhoneIntent.putExtra("phone", phone);
            startActivity(getPhoneIntent);

        }else if(phone.length()!=11 || yzCode.length()!=6){ // 输入不合法
            Toast.makeText(RegisterActivity.this,"手机号或验证码输入不合法",Toast.LENGTH_SHORT).show();
        }else { //验证失败
