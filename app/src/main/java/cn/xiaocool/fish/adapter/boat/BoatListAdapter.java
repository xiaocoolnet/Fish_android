...
public class BoatListAdapter extends BaseAdapter implements AbsListView.OnScrollListener{
	...
	private BoatImageLoader mImageLoader;
	...
	public BoatListAdapter(Context context, List<BoatListBean> data, ListView listView) {
		mInflater = LayoutInflater.from(context);
		this.mList = data;
		mImageLoader = new BoatImageLoader(listView); // 在这里初始化，能够保证只有一个imageloader的实例，即只有一个LruCache的实例
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			convertView = mInflater.inflate(R.layout.activity_boatfish_list, null);
		...
		// new BoatImageLoader().showImageByThread(holder.ivIcon, mList.get(position).getNewsIconUrl());
